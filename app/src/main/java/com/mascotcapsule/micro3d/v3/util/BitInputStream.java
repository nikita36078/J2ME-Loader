...
public class BitInputStream implements Closeable {
	 ...
	 */
	public long readBitsLong(final int count) throws IOException {
	...
	public int readBitsSigned(final int count) throws IOException {
		int value = readBits(count);
		int sign = value & (1 << (count - 1));

		if (sign > 0) value -= (1 << count);
		return value;
	}
